...
soc1 <- labour %>%
  ...
  filter(Gender == "Total - Gender" & `Age group` == "15 years and over") %>%
...
soc2 <- labour %>%
  ...
  filter(`Classification Code for North American Industry Classification System (NAICS)` %in% labour_NAICS |
           `North American Industry Classification System (NAICS)` == "Total, all industries") %>%
  filter(`Labour force characteristics` == "Employment" & Gender == "Total - Gender") %>%
  select(Year = REF_DATE, Region = GEO, Industry = `North American Industry Classification System (NAICS)`,
         `Age group`, val_norm) %>%
...
soc7 <- pay %>%
  ...
  rename(Region = GEO,
         Industry = `North American Industry Classification System (NAICS)`) %>%
  group_by(Year, Region, Industry, Gender) %>%
  summarize(Value = mean(val_norm), .groups = "drop") %>%
  pivot_wider(names_from = "Gender", values_from = "Value") %>%
  mutate(Value = 100*(`Men+` - `Women+`)/`Men+`,
         Unit = "%",
...
table_indicators <- list(env7, env8, soc1, soc2, soc3, soc4, soc5, soc7, soc8, soc10, soc11)
if(nrcan) table_indicators <- list(env1, env4) %>% append(table_indicators)
table_data <- map_dfr(table_indicators, format_for_tables)
rm(table_indicators)
...
table_data %>% 
  ...
  saveRDS("01_data/table_data.rds")
...
wkabsence <- absences %>%
    filter(REF_DATE >= 2000 & Gender != "Total - Gender") %>%
    ...
    mutate(Metric = "Work absences by gender and presence of children",
